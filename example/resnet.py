...
# Training setup
scaler = GradScaler()
model = RoundPipe(ResNet().seq.to(torch.float16), optim_dtype=torch.float32)
with OptimizerCtx():
...
for epoch in range(epochs):
    ...
    for images, labels in trainloader:
        ...
        model.step(lambda: (scaler.step(optimizer), optimizer.zero_grad())[1])
        scaler.update()
...
with open(os.path.join(script_dir, "resnet_cifar100_ref.log"), "r") as f:
    for line in f:
        ...
        if match:
            ref_train_losses.append(float(match.group(1)))
            ref_train_accs.append(float(match.group(2)))
            ref_test_losses.append(float(match.group(3)))
