...
def run_optim(cls: type[torch.optim.Optimizer], ref_cls: type[torch.optim.Optimizer],
    ...
    for n in range(1, 11):
        params.append(torch.randn(n, n + 1, n + 2, dtype=torch.float32, requires_grad=True))
    for n in range(1025):
...
@pytest.mark.parametrize("lr_betas, weight_decay, amsgrad, maximize, decoupled_weight_decay",
    itertools.product([(0.01, 0.9, 0.999), (torch.tensor(0.001), torch.tensor(0.8), torch.tensor(0.95))],
                      [0.0, 0.01], [True, False], [True, False], [True, False]))
def test_Adam(lr_betas: Union[Tuple[float, float, float],
