...
class RoundPipeRunConfig:
    ...
    def __init__(
        ...
        split_input: Union[
            ...
            Callable[
                [Tuple, Dict[str, Any], int], Tuple[List[Tuple], List[Dict[str, Any]]]
            ],
            None,
        ] = None,
    ...
    def __init__(
        ...
        self.requires_grad = (
            ...
            if function_run_config.requires_grad is not None
                ...
                model_run_config.requires_grad
                if model_run_config.requires_grad is not None
                else torch.is_grad_enabled()
            )
        )
        self.output_device = (
            function_run_config.output_device
            if function_run_config.output_device is not None
            else (
                model_run_config.output_device
                if model_run_config.output_device is not None
                else torch.device("cpu")
            )
        )
        self.preserve_rng_state = (
            function_run_config.preserve_rng_state
            if function_run_config.preserve_rng_state is not None
            else (
        ...
        self.num_microbatch = (
            function_run_config.num_microbatch
            if function_run_config.num_microbatch is not None
            else (
        ...
        self.split_input = (
            function_run_config.split_input
            if function_run_config.split_input is not None
            else (
                model_run_config.split_input
                if model_run_config.split_input is not None
                else (None, None)
            )
        )
        self.split_label = (
            function_run_config.split_label
            if function_run_config.split_label is not None
