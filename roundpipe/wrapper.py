...
def wrap_model_recursive(
    ...
    **roundpipe_kwargs: Any,
    ...
    if (
        lower_threshold <= get_model_size(model, False)
        or get_model_size(model) <= upper_threshold
        ...
        return RoundPipe(
            model, name=name, model_run_config=model_run_config, **roundpipe_kwargs
        )
        ...
        for layer_idx in range(n_layers - 1):
            model[layer_idx] = RoundPipe(
                ...
                model_run_config=override_config.get(
                    model[layer_idx], model_run_config
                ),
                **roundpipe_kwargs,
            )
        return model
